...
const Navbar = () => {
  ...
  const total = useMemo(
    () => cart.reduce((prev, curr) => curr.price * curr.quantity + prev, 0),
    [cart]
  );
  ...
  return (
    <div className="navbar sticky top-0 z-10 bg-slate-200 shadow-lg dark:bg-slate-900 lg:pr-3">
      <div className="flex-1">
        <Link href="/">Sunnah Weather</Link>
      </div>
      <div
      ...
      >
        ...
        {uid && (
          <div className="dropdown dropdown-end">
            ...
            >
              <li className="mb-2 mt-1 text-center">{displayName}</li>
      ...
      <label className="swap-rotate swap btn-ghost btn-circle btn ml-2 bg-white dark:bg-slate-800 lg:hidden">
        <input onChange={() => setNavToggle((pre) => !pre)} type="checkbox" />
        <svg
